...
Luego, a diferencia de los archivos generados para el reporte, los archivos a generar para la calibración no cuentan con un archivo de delimitación de cuenca hasta los puntos donde existe una estación hidrométrica con información. Por este motivo se debe generar los archivos de delimitación a partir de los archivos de delimitación de reporte. Para este trabajo sera de utilidad el archivo *.csv* mostrado en la figura \ref{fig:csv_q_big}. 

<!-- \pagebreak -->

Con la función `calibr()` se crean los archivos de delimitación de las cuencas de calibración y se preparan los archivos de calibración. Si es necesario corregir por desfase temporal usar la función `csv_q_visor()` antes que la función `calibr()`.

```{r calibr, eval=FALSE}
#' @author Eduardo Noriega
#' 
#' @description La funcion permite crear un shapefile de calibracion en base a un 
#' shapefile de reporte de un balance hidrico
#' 
...
#' b = nombre rio donde esta la estacion hidrometrica
#' c = Nodo (de WEAP) de donde se extrae informacion de caudal 
#' d = Existe estacion hidrometrica para nodo del que se extrae informacion? 
#' Binario (1 o 0)
...
#' 
#' @details dentro de la estructura de la funcion se presenta una funcion 
#' adicional llamada 'uh_calibr()'
#' Esta funcion permite asociar la UH del reporte con las UH de la calibracion de 
#' forma interactiva. Al ser llamada, la funcion itera a traves de la cantidad de 
#' datos (cantidad de uh de calibracion) del archivo 'datos_resumen'. Para cada dato 
#' se pide intrducir el numero de UH del reporte que le corresponde a cada UH de calibracion 
...
library(sf)
library(ggplot2)
base <- st_read("C:/Users/HP/Documents/WEAP Areas/Balance Hidrico Ravelo FINAL/bh/shp/ravelo_calibr_solo2.shp",
                quiet = TRUE)
base_ <- st_set_geometry(base, NULL)
# par(mfrow = c(1,2))
ggplot(base) + 
