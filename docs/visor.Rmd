...
A partir de lo anterior se deduce la importancia de contar con información accesible para la sociedad y espacialmente para los sectores que interactúan con agua dentro de sus actividades económicas y/o sociales.
...
# Variables de reporte y calibración
...
Para *PCP*, *ETP*, *ETR* y *esc* se presentarán los resultados en $[mm]$ por unidad hidrológica modelada, tomando en cuenta que el Escurrimiento [*esc*] se considera como la sumatoria del escurrimientos superficial **[SR]**, subsuperficial **[IN]** y base **[BF]**. 
...
## Resultados en lámina de precipitación $[mm]$

Se presentaran los resultados de este tipo de variables según el cuadro \ref{tab:tabla_formato_mm_csv}.

```{r ej_reporte, echo=FALSE}
ej_reporte <- head(read.csv("H:/mmaya/proyectos_R/visor_git/docs/PCP_visor.csv"))
knitr::kable(ej_reporte, caption = "\\label{tab:tabla_formato_mm_csv}Formato para archivos csv para vairables en $[mm]$.")
```

Se incluirán las columnas `Year`, `Month` y el código de cada unidad hidrológica. Este código deberá ser la concatenación entre el termino **uh** y un numero único que identifique a cada unidad hidrológica, y deberá corresponder a cada subcuenca del archivo de delimitación espacial.

## Resultados para caudal $[m^3/s]$

Se presentarán los resultados de este tipo de variables según el cuadro \ref{tab:tabla_formato_mm3_S_csv}.
...
3. `OBSERV_`, 
4. `codigo` con la concatenación del termino **uh** y un numero único de identificación de la subcuenca.
5. `Cuenca` idéntica a `NOMBRE`. 
